...
Сессии диалогов — каталог `/var/lib/ai-family/chats/` на хосте. Личный чат — файл `user:<id>`, тема группы — `topic:<chat>:<тема>`. В файле id чата Cursor, `startedAt` и `updatedAt`. Это состояние машины, в git его нет. Воркер создаёт отдельную рабочую копию проекта для каждого контекста в `/var/lib/ai-family/repos/contexts/` и обновляет её перед заданием. Личные диалоги используют `/var/lib/ai-family/workspace/contexts/`. Вложения из Telegram попадают в `inbox/` своей копии. По умолчанию одновременно выполняется не больше трёх задач; лимит задаёт `MAX_CONCURRENT_AGENTS` в `/etc/ai-family.env`. Витрину выкладывает не этот репозиторий, а пуш в `main` репозитория проекта.

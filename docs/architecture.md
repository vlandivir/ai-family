...
GitHub Actions тестирует воркер и обновляет его после изменений в `worker/` или `scripts/`; правки сайта статусов воркер не перезапускают. Сервер готовит версию в отдельном каталоге `/opt/ai-family-releases/<SHA>`, затем переключает ссылку `/opt/ai-family-current` и перезапускает службу. Если новая служба сразу падает, ссылка и unit возвращаются на прежнюю версию. При необходимости развёртывание можно запустить вручную.
...
Отдельный сайт статусов в `dashboard/` этого репозитория показывает список диалогов и переписку в каждой ветке. Входящее сообщение берётся из `agent_jobs.payload`, ответ агента — из `agent_jobs.result.text`; очередь и ошибки видны рядом с сообщением. Он размещается как отдельный проект Vercel с корнем `dashboard/`. Вход выполняется через Google и Supabase Auth; доступ к данным проверяется на сервере по `ALLOWED_EMAILS`. Ключ service role доступен только серверному коду. Адреса хранятся в игнорируемом `.env` и копируются в переменные окружения Vercel.

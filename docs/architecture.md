...
Приём Telegram-сообщений отделён от работы агента: бот записывает сообщение в `agent_jobs` со статусом `queued` и сразу подтверждает получение. Диспетчер забирает задачи из базы по порядку. В одном личном диалоге или теме задачи идут последовательно; разные контексты работают параллельно, по умолчанию не больше трёх запусков Cursor CLI. Лимит меняется через `MAX_CONCURRENT_AGENTS`. Ожидающие задачи сохраняются при перезапуске. Прерванная активная задача помечается ошибкой, и бот просит отправить сообщение повторно.
